      ...
      if (sekme === 'giris') {
        ...
        setTimeout(() => {
          ...
          if (form.email.trim().toLowerCase() === adminEmail) {
            navigate('/') // Yöneticiyse React içindeki Dashboard'a ışınla
          } else {
            window.location.href = 'https://lojistikweb-vitrin.vercel.app/' // Normal kullanıcıyı Vitrin'e şutla
          }
        }, 1000)
        ...
        // Kayıt başarılı → Dashboard'a değil, direkt Vitrin'e yönlendir
        setTimeout(() => {
          window.location.href = 'https://lojistikweb-vitrin.vercel.app/'
        }, 1200)
      }
    } catch (err) {
      setHata(err.message)
